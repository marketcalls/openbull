/**
 * API Playground — REST + WebSocket tester.
 *
 * Ported from openalgo. Adaptations for openbull:
 *  - Auth: cookie-JWT instead of Zustand authStore + CSRF.
 *  - Mode: useTradingMode() (Live | Sandbox) instead of useThemeStore.appMode.
 *  - API key + endpoints + host fetched via /web/playground/* helpers.
 *  - Branding: openbull wordmark, drop logo/image references.
 *  - URL guardrail allows /api/v1/, /web/, and the playground helpers.
 */
...
import {
  getPlaygroundApiKey,
  getPlaygroundEndpoints,
  getPlaygroundHost,
  type PlaygroundEndpoint,
  type PlaygroundEndpointsByCategory,
} from "@/api/playground";
...
function tokenizeJson(json: string): SyntaxToken[] {
  if (json.length > 100 * 1024) return [{ text: json, type: "plain" }];
  ...
  while (match !== null) {
    ...
    if (/^"/.test(text)) type = /:$/.test(text) ? "key" : "string";
    else if (/true|false/.test(text)) type = "boolean";
    else if (/null/.test(text)) type = "null";
    tokens.push({ text, type });
    lastIndex = regex.lastIndex;
    match = regex.exec(json);
  }
  if (lastIndex < json.length) tokens.push({ text: json.slice(lastIndex), type: "plain" });
  return tokens;
}
...
function isValidApiUrl(url: string, method?: string): { valid: boolean; error?: string } {
  // WebSocket URLs go through the proxy direct — let them through for WS.
  if (method === "WS" && (url.startsWith("ws://") || url.startsWith("wss://"))) {
    return { valid: true };
  }
  if (
    url.startsWith("/api/") ||
    url.startsWith("/web/playground/") ||
    url.startsWith("/web/")
  ) {
    return { valid: true };
  }
  try {
    ...
    if (
      ...
      return {
        valid: false,
        error: "Only /api/ and /web/ endpoints are allowed",
      };
    }
    return { valid: true };
  } catch {
...
export default function Playground() {
  ...
  const { mode, isSandbox, setMode } = useTradingMode();
  ...
  const [endpoints, setEndpoints] = useState<EndpointsByCategory | Record<string, Endpoint[]>>(
    {},
  );
  const [collapsedCategories, setCollapsedCategories] = useState<Set<string>>(
    new Set(["data", "analytics", "utilities", "websocket"]),
  );
  ...
  const [method, setMethod] = useState<"GET" | "POST" | "WS" | "PUT" | "DELETE" | "PATCH">(
    "POST",
  );
  ...
  useEffect(() => {
    getPlaygroundHost().then(setHostServer);
    getPlaygroundApiKey().then(setApiKey);
    getPlaygroundEndpoints()
  ...
  const getDefaultBody = useCallback(
    (endpoint: Endpoint): string => {
      ...
      if (apiKey && !body.apikey) body.apikey = apiKey;
  ...
  const selectEndpoint = (endpoint: Endpoint) => {
    if (endpoint.method === "WS") {
      ...
      const bodyWithApiKey = apiKey
        ? body.replace(/"api_key":\s*""/, `"api_key": "${apiKey}"`)
        : body;
  ...
  const prettifyBody = () => {
    try {
      ...
      updateCurrentTabBody(JSON.stringify(parsed, null, 2));
  ...
  const handleModeToggle = async () => {
    try {
      const next = mode === "live" ? "sandbox" : "live";
      await setMode(next);
      toast.success(
        `Switched to ${next === "live" ? "Live" : "Sandbox"} mode`,
      );
      if (next === "sandbox") {
        setTimeout(() => {
          toast.warning(
            "Sandbox mode is for simulated trading — orders won't reach the broker.",
            { duration: 10000 },
          );
        }, 1500);
      }
      ...
      <div className="h-12 border-b border-border flex items-center px-2 bg-card/50">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link to="/dashboard" className="flex items-center gap-2 px-2">
            ...
            <span className="font-semibold text-sm">openbull</span>
          </Link>
        </div>
        ...
        <div className="flex-1 flex items-center gap-1 px-4 overflow-x-auto">
          ...
          <Button
            ...
            onClick={() => {
              ...
              const eps = (endpoints as Record<string, Endpoint[]>)[firstCat];
              if (eps?.[0]) selectEndpoint(eps[0]);
            }}
          >
        ...
        {/* Right — mode toggles + profile */}
        <div className="flex items-center gap-2 px-2">
          {/* REST / WebSocket mode toggle */}
          ...
          <Badge
            ...
            className={cn(
              "text-xs",
              isSandbox && "bg-amber-500 hover:bg-amber-600 text-white",
            )}
          >
            <span className="hidden sm:inline">
              {isSandbox ? "Sandbox Mode" : "Live Mode"}
            </span>
            <span className="sm:hidden">{isSandbox ? "Sandbox" : "Live"}</span>
          </Badge>
          ...
          <Button
            ...
            title={`Switch to ${isSandbox ? "Live" : "Sandbox"} mode`}
          >
            {isSandbox ? (
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56">
              <DropdownMenuItem onClick={() => navigate("/apikey")}>
                <Key className="h-4 w-4 mr-2" />
                API Key
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => navigate("/broker/config")}>
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              {activeTab ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center bg-background">
                  <div className="w-16 h-16 mb-4 rounded-2xl bg-primary text-primary-foreground flex items-center justify-center text-2xl font-bold">
                    OB
                  </div>
